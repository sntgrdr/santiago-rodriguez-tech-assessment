      ...
      context "when source is #{source}" do
        ...
        context 'when the person does not exist' do
          it 'creates a new person' do
            ...
            person = service_call
            expect(person.email).to eq(payload[:email])
          end
        end

        context 'when the person already exists' do
          let!(:person) { Person.create!(first_name: 'Jane', last_name: 'Doe', email: payload[:email]) }

          it 'updates the existing person' do
            expect { service_call }.not_to change(Person, :count)

            person.reload
            expect(person.first_name).to eq(payload[:first_name])
            expect(person.last_name).to eq(payload[:last_name])
          end
        end
        ...
        context 'when the external identity already exists' do
          ...
          it 'updates last_synced_at' do
            expect { service_call }
              .to change { identity.reload.last_synced_at }
          end

          it 'does not create a duplicate identity' do
            expect { service_call }
              .not_to change(ExternalIdentity, :count)
          end
        end
      end
    end
